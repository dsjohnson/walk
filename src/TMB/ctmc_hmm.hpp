...
template<class Type>
Type ctmc_hmm(objective_function<Type>* obj) 
{
  // Data
  ...
  // Project backwards
  BackwardProb_gt.col(n_t-1) = L_gt.col(n_t-1);
  for( int t=n_t-2; t>=0; t-- ){
    ...
    // Calculate probability (and re-normalize to avoid accumulating underflow in likelihood)
    BackwardProb_gt.col(t) = BackwardPred_gt.col(t).array() * L_gt.col(t).array();
    BackwardProb_gt.col(t) = BackwardProb_gt.col(t) / BackwardProb_gt.col(t).sum();
  }
