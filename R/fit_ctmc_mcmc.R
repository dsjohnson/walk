...
#' #' @export
#' fit_ctmc_mcmc <- function(walk_data, 
#'                      model_parameters = ctmc_control(), 
#'                      pen_fun, reals=FALSE, start=NULL,  
...
#'   post_fun <- function(par){-0.5*ctmc_n2ll(par, data_list, debug=0)}
#'   
#'   ppp <- MCMC(post_fun, n=1000, init=par_start, scale=diag(length(par_start)), acc.rate=0.234, adapt = TRUE)
#'   
#'   
#'   
#'   if(debug==2) browser()
